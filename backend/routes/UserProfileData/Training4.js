...
router.post('/updatelock', fetchuser, isAdmin, async (req, res) => {
    try {
        ...
        const newLogEntry = new logEntry({
            user: adminCrn,
            logMessage: `Verified Status for ${crn} is updated to ${lock} in tr104`
        });
        newLogEntry.save()
...
router.get('/:crn', fetchuser, async (req, res) => {
    try {
        ...
        const userInfo = await SignUpdata.findOne({ crn: crn }).populate('tr104');
...
router.post('/verifyall', fetchuser, isAdmin, async (req, res) => {
    try {
        ...
        const updatedUsers = await Promise.all(usersToUpdate.map(async (user) => {
            try {
                if (user.tr104) {
                    user.tr104.lock = true; // Set lock status to true
                    await user.save();
                }
                return user;
            } catch (err) {
        ...
        const newLogEntry = new logEntry({
            user: adminCrn,
            logMessage: `Verified Status for all Students of ${batch} is updated to true for tr104`
        });
        newLogEntry.save()
        // Respond with the updated user data
        res.status(200).json({ success: true });
...
router.post('/unverifyall', fetchuser, isAdmin, async (req, res) => {
    try {
        ...
        const updatedUsers = await Promise.all(usersToUpdate.map(async (user) => {
            try {
                if (user.tr104) {
                    user.tr104.lock = false; // Set lock status to true
                    await user.save();
                }
                return user;
            } catch (err) {
        ...
        const newLogEntry = new logEntry({
            user: adminCrn,
            logMessage: `Verified Status for all Students of ${batch} is updated to false for tr104`
        });
        newLogEntry.save()
        // Respond with the updated user data
        res.status(200).json({ success: true });
