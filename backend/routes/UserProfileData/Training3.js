...
router.post('/updatelock', fetchuser, isAdmin, async (req, res) => {
    try {
        ...
        const newLogEntry = new logEntry({
            user: adminCrn,
            logMessage: `Verified Status for ${crn} is updated to ${lock} in tr103`
        });
        newLogEntry.save()
...
router.post('/verifyall', fetchuser, isAdmin, async (req, res) => {
    try {
        ...
        const newLogEntry = new logEntry({
            user: adminCrn,
            logMessage: `Verified Status for all Students of ${batch} is updated to true for tr103`
        });
        newLogEntry.save()
        // Respond with the updated user data
        res.status(200).json({ success: true });
...
router.post('/unverifyall', fetchuser, isAdmin, async (req, res) => {
    try {
        ...
        const newLogEntry = new logEntry({
            user: adminCrn,
            logMessage: `Verified Status for all Students of ${batch} is updated to false for tr103`
        });
        newLogEntry.save()
        // Respond with the updated user data
        res.status(200).json({ success: true });
